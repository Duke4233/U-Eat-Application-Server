...
router.get('/:resid', function(req, res, next)
{
	...
	{
		db.query("SELECT restaurant.name AS resname, restaurant.id AS resid, menu_item.imageURL, menu_item.name, menu_item.price, NULL AS username, NULL AS rating, NULL AS review " +
			"FROM restaurant INNER JOIN menu_item ON restaurant.id = menu_item.restaurant_id " +
			"WHERE restaurant.id = ?;" +
			"SELECT restaurant.name AS resname, restaurant.id AS resid, NULL AS imageURL, NULL AS name, NULL AS price,account.username, review.rating, review.review " +
